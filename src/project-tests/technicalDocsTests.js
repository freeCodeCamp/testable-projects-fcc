  ...
  describe("Technical Documentation Page tests", function() {
    describe('#Content', function() {
      it('1. I can see an <article> element with a corresponding id="main-doc", which contains the ' + 
         'page\'s main content (technical documentation).', function() {
      ...
      it('2. Within the #main-doc <article> element, I can see several <section> elements, each with a class ' +
         'of "main-section". There should be a minimum of 5.', function() {
        let classQty = classArray('main-section').length;
        let typeQty = classArray('main-section').filter(el => el.nodeName === 'SECTION');
        FCC_Global.assert.isAbove(document.querySelectorAll('#main-doc .main-section').length, 0, 'There are no .main-section ' +
        'elements within #main-doc ');
        FCC_Global.assert.isAtLeast(document.querySelectorAll('#main-doc .main-section').length, 5, 'There are not at least ' +
        '5 elements with the class of "main-section" ');
        FCC_Global.assert.strictEqual(document.querySelectorAll('.main-section').length, 
        document.querySelectorAll('#main-doc .main-section').length, 'All of the page\'s .main-section elements should be ' +
        'within #main-doc ');
        FCC_Global.assert.strictEqual(typeQty.length, classQty, 'Not all of the elements with the class of "main-section" are ' +
        '<section> elements ');
      });
      
      it('3. The first element within each .main-section should be a <header> element which contains text that ' + 
         'describes the topic of that section.', function() {
        let classQty = document.getElementsByClassName('main-section').length;
      ...
      it('4. Each <section> element with the class of "main-section" should also have an id that corresponds with the ' +
         ...
         'id="Javascript_and_Java").', function() {
        ...
        const headerText = mainSections.map((el, i) => [][i] = el.firstElementChild.innerText.replace(/\s/g, '_'));
        const mainSectionIDs = mainSections.map((el, i) => [][i] = el.id);
        const remainder = headerText.filter(str => mainSectionIDs.indexOf(str) === -1);
        FCC_Global.assert.strictEqual(remainder.length, 0, 'Some "main-section" elements are missing the following ids (don\'t ' +
        'forget to replace spaces with underscores!) : ' + remainder + ' ');
      });
      ...
      it('8. I can see a <nav> element with a corresponding id="navbar".', function() {
        ...
        FCC_Global.assert.strictEqual(document.getElementById('navbar').nodeName, "NAV", 'The element with the id of ' +
        '"navbar" is not a <nav> element ');
      });
      
      it('9. The first element within the navbar should be a <header> which contains text that describes the topic ' +
         'of the technical documentation.', function() {
        FCC_Global.assert.strictEqual(document.getElementById('navbar').firstElementChild.nodeName, 'HEADER', 'The first ' + 
        'element within #navbar should be a <header> ');
      });
      
      it('10. Additionally, the navbar should contain link (<a>) elements with the class of "nav-link". There ' +
         'should be one for every element with the class "main-section".' , function() {
        ...
        FCC_Global.assert.isAbove(navLinkQty, 0, 'The element with the id of "navbar" does not contain any elements with the ' +
        'class of "nav-link" ');
        FCC_Global.assert.strictEqual(navLinkQty, mainSectionQty, 'There should be one .nav-link for every element ' +
        'with the class of "main-section", and every .nav-link should be within #navbar ');
        FCC_Global.assert.strictEqual(typeQty.length, navLinkQty, 'Not all of the elements with the class of "nav-link" are ' +
        '<a> elements ');
      });
      
      it('11. Each element with the class of "nav-link" should contain text that corresponds to the <header> text ' +
        ...
        'be mindful of case! : ' + remainder + ' ');
      });
      
      it('12. When I click on a navbar element, the page should navigate to the corresponding section of the ' + 
         '"main-doc" element (e.g. If I click on a "nav-link" element that contains the text "Hello world", the page ' +
         'navigates to a <section> element that has that id and contains the corresponding <header>.', function() {
        ...
        'that links it to its corresponding .main-section (e.g. href="#Introduction"). Check that these ' +
        '.main-section ids have corresponding href values : ' + missingHrefValues + ' ');
      });
    }); // END #Content
