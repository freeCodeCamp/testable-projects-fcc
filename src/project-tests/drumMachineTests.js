  ...
  describe('#Drum Machine tests', function() {
    ...
    describe('#Tests', function() {
      ...
      it('3. Within #drum-machine I can see 9 clickable "drum pad" elements, each with a class name of "drum-pad", ' +
         ...
         'corresponds to one of the following keys on the keyboard: Q, W, E, A, S. D, Z, X, C.', function() {
        ...
        FCC_Global.assert.includeMembers(
          drumPadInnerText, // superset
          ["Q", "W", "E", "A", "S", "D", "Z", "X", "C"], // subset
          'Each .drum-pad\'s inner text should be one of the following letters ' +
          '(all letters must be represented): "Q", "W", "E", "A", "S", "D", "Z", "X", "C" '
        );
      });
      ...
      it('4. Within each .drum-pad, there should be an HTML5 <audio> element which has a src attribute pointing to ' +
         ...
         '(e.g. id="Q", id="W", id="E" etc.).', function(){
      ...
      it('5. When I click on a .drum-pad element, the audio clip contained in its child <audio> element ' +
        ...
        audioElements.forEach( el => {
          ...
          FCC_Global.assert.isNotOk(document.getElementById(el.id).paused, 'The <audio> element with id="' + el.id  + '" does not play when the ' 
          + el.id + ' .drum-pad is clicked ');
        });
      });
      ...
      it('7. When a .drum-pad is triggered, a string describing the associated audio clip is displayed as ' +
         'the inner text of the .display element (each string must be unique).', function() {
        this.timeout(900);
        let displayText = [];
        return new Promise((resolve, reject) => {
          setTimeout( _ => {
