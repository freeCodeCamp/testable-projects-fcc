...
the development server (see below).
...
```
> webpack-dev-server --config webpack.config.develop.js
...
You can leave this command running, and every time you change files in `src/`,
it will automatically detect the change, compile and reload `bundle.js`.

**NOTE** Your changes will not be saved to the bundle we're using for production until
...
NOTE: If the address used by `yarn develop` from the section above
...
- Copy `local_test` directory somewhere.
- Import the code from the
[official FCC example project](http://codepen.io/collection/npZPmR) that you
are creating/editing the test suite for into the copied `local_test` directory:
- Create folders for your code, i.e. JS, CSS, etc.
...
- Run `yarn develop` to start watching your files for changes and serve bundle.js.
The bundle is served at https://localhost:8080/bundle.js after adding the security
